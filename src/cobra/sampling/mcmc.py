...
class MCMCACHRSampler(HRSampler):
    ...
    def __single_iteration(self, lockCenter=False, validatecheck=False, savePrev=None):
        ...
        if validatecheck and np.any(savePrev)!=None:
            ...
            #    print('valid')
            while counter<=nmax:
                if "v" not in any(str(s) for s in self.validate(np.transpose(self.testprev), feas_tol=1e-7, bounds_tol=1e-7)):#first sample: #input have to be netsamples and in form samples x reactions
                    if counter==nmax:
