...
class MCMCACHRSampler(HRSampler):
    ...
    def __single_iteration(self, lockCenter=False, validatecheck=False, savePrev=None):
        ...
        if validatecheck and np.all(savePrev)!=False:
            ...
            #    print('valid')
            while counter<=nmax and np.all(self.validate(np.transpose(self.testprev), feas_tol=1e-7, bounds_tol=1e-7))!='v':#first sample: #input have to be netsamples and in form samples x reactions
                if counter==nmax:
