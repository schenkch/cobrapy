...
class MCMCACHRSampler(HRSampler):
    ...
    def __single_iteration(self, lockCenter=False, validatecheck=False, savePrev=None):
        ...
        if validatecheck and np.any(savePrev)!=None:
            ...
            #    print('valid')
            while counter<=nmax:
                if all(self.validate(np.transpose(self.testprev)))!="v":#first sample: #input have to be netsamples and in form samples x reactions#, feas_tol=1e-6, bounds_tol=1e-6)
                    if counter==nmax:
