...
class MCMCACHRSampler(HRSampler):
    ...
    def __single_iteration(self, lockCenter=False, validatecheck=False, savePrev=False):
        ...
        if validatecheck:
            ...
            #    print('valid')
            while counter<=nmax and ['v']!=np.any(self.validate(np.transpose(self.testprev), feas_tol=1e-7, bounds_tol=1e-7))) and savePrev!=False:#first sample: #input have to be netsamples and in form samples x reactions
                if counter==nmax:
